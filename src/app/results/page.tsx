...
export default function ResultsPage() {
  ...
  if (!isClient) {
    // This state is primarily for the initial render before client-side hydration
    feedbackMessage = "Loading results...";
    feedbackIcon = <HelpCircle className="h-16 w-16 text-muted-foreground" />;
    scoreColorClass = "text-muted-foreground";
  } else if (correctStr === null || incorrectStr === null || totalStr === null) {
    // This case is handled by an early return block below, but initializing for type safety
    feedbackMessage = "Error: Could not load quiz results.";
  ...
  } else { // totalNum is 0 or less, but params are present
     ...
     return (
      <main className="flex min-h-screen flex-col items-center justify-center p-6 text-center bg-gradient-to-br from-background to-secondary dark:from-slate-900 dark:to-slate-800">
        <Card className="w-full max-w-md shadow-xl">
          ...
          <CardContent>
            <p className="text-muted-foreground">Could not load quiz results. Please try playing again.</p>
          </CardContent>
          <CardFooter>
